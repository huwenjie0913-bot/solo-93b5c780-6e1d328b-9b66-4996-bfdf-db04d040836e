...
- **可用容量**：短板原则，取组内最小单体容量（Ah）；同时给出估算整包电压
  （最低 OCV × 串联数）与估算整包内阻（平均内阻 / P × S）。
- **最弱电芯**：容量最低，其次内阻最高、SOH 最低；并给出相对组容量均值的缺口百分比。
- **风险评分（0–100）**：容量/内阻 CV 越限、温差、OCV 差、最低 SOH、串位置 SOC
  不均衡估算分项累计；尾料 / 未满配直接 CRITICAL(100)。等级：LOW <25，MEDIUM <55，
  HIGH <80，CRITICAL ≥80。每条原因带 `code / message / measured / threshold`。
